...
kvm_root_path=/usr/lib/python3/dist-packages/kvmd/apps

kvmd_path=$kvm_root_path/kvmd/api
web_js_path=/usr/share/kvmd/web/share/js/kvm
web_path=/usr/share/kvmd/web/kvm
htpaswd_path=$kvm_root_path/htpasswd
...
cp -rv $htpaswd_path/__init__.py $htpaswd_path/__init__.py_$current_timestamp || exit -1

echo "更新补丁文件到系统"
cp -rv info.py $kvmd_path/ || exit -1
cp -rv recorder.js $web_js_path/ || exit -1
cp -rv index.html $web_path/ || exit -1
cp -rv __init__.py $htpaswd_path/ || exit -1
